...
버전이 나오면 설치된 것입니다. 로그인 여부는 아래로 확인합니다.

```bash
claude auth status
```

`"loggedIn": true` 가 아니면 `tools\목회AI_로그인.bat` 을 더블클릭해(또는
`claude auth login`) 로그인해 두세요. (API 키가 아니라 **구독 로그인**입니다)
...
## "교회 컴퓨터의 Claude 로그인이 풀렸습니다" 가 뜬다

교회 PC 의 claude CLI **구독 로그인이 풀린** 것입니다(토큰 만료·계정 기기 정리 등으로
가끔 일어납니다). 이때는 기도문·헤드라인·주보 검수·말씀지기 모두 답을 만들지 못하며,
워커 창에도 `!!  claude CLI 로그인이 풀렸습니다` 경고가 크게 뜹니다.
(말씀지기 화면의 교인에게는 "말씀지기가 잠시 쉬고 있어요"로 보입니다)

1. 교회 PC 에서 `tools\목회AI_로그인.bat` 을 더블클릭합니다.
2. 브라우저가 열리면 교회 Claude 계정(구독)으로 로그인합니다.
3. 창에 `"loggedIn": true` 가 보이면 끝입니다. **워커를 다시 켤 필요 없이** 다음 요청부터
   바로 정상 동작합니다.

2026-09-16: 로그인이 풀린 채로 화면에 "Not logged in · Please run /login" 영어 오류만 떠서
원인을 알기 어려웠던 일 이후, 워커가 이 상황을 감지해 위 문구로 안내하고 워커 창에서도
10분마다 로그인 상태를 확인하도록 고쳤습니다.
