...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!content.trim()) return new Response(JSON.stringify({ error: "검수할 주보 내용이 비어 있습니다." }), { status: 400, headers: { ...cors, "Content-Type": "application/json" } });
    ...
    const userMsg = "다음은 이번 주 주보 초안입니다. 위 4가지를 점검해 주세요.\n\n" + content;
    let reply = "";
    ...
    if (PROVIDER === "gemini") {
      ...
      const reqBody = JSON.stringify({ model: MODEL, max_tokens: 2048, system: SYSTEM_PROMPT, messages: [{ role: "user", content: userMsg }] });
      const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));
      let lastErr = "";
